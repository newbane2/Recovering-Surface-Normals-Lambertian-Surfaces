/*
    ...
    The albedo is the Normalization constant (magnitude of the surface normal vector ||N||)
    Generate an albedo map for each pixel above the threshold
...
Image computeSurfaceNormals(vector<Image> & images, matrix3x3 & inverseMatrix, int threshold);
...
int main(int argc, char **argv) 
{
    ...
    Image output = computeSurfaceNormals(images, inverseMatrix, threshold);
    ...
    for(int i = 0; i < images[0].num_rows(); i++) {
        for(int j = 0; j < images[0].num_columns(); j++) {
            if(checkThreshold(images, threshold, i, j)) {
                //calc and draw surface normal :shrug:, probably going to have to calc and then draw 2 diff funcs
                NormalVector surfaceNormal = multiplyInverseByIrradiance(inverseMatrix, generateIrradianceVector(images, i, j));
                double normalizationConstant = sqrt(pow(surfaceNormal.x,2) + pow(surfaceNormal.y,2) + pow(surfaceNormal.z,2));
               // cout << normalizationConstant << endl;
                ret.SetPixel(i, j, normalizationConstant * 100);
            } else {
                ret.SetPixel(i,j,0);
            }
        }
